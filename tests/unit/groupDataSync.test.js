...
describe('groupDataSync', () => {
  ...
  it('accepts an opt-in profile.notes pack without private share targets', () => {
    const pubkey = '02' + 'ab'.repeat(32);
    const share = sync.buildShare({
      groupId: 'grp1',
      packs: [{
        pack: sync.PACK_PROFILE_NOTES,
        payload: {
          pubkey,
          notes: [{
            id: 'note-abc',
    ...
    assert.ok(share);
    assert.strictEqual(share.packs.length, 1);
    assert.strictEqual(share.packs[0].pack, sync.PACK_PROFILE_NOTES);
    assert.strictEqual(share.packs[0].payload.pubkey, pubkey);
    assert.strictEqual(share.packs[0].payload.notes[0].body, 'Public warning');
    assert.strictEqual(share.packs[0].payload.notes[0].subject, 'discord:u1');
    assert.ok(!share.packs[0].payload.notes[0].sharePeerPubkey);
  });
  ...
  it('composes a world view from catalog + message stats + playtimes', () => {
    ...
    const notesPack = view.packs.find((p) => p.pack === sync.PACK_PROFILE_NOTES);
    ...
    assert.strictEqual(notesPack.profileCount, 1);
